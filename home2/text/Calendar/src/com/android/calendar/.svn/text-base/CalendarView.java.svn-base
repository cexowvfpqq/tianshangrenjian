...
public class CalendarView extends View
    ...
    private boolean initNextView(int deltaX) {
        ...
        if (deltaX > 0) {
        	judgeDate(view, date, mNumDays, mSelectionDay, false); // chenzhentao 2012-5-17 modify
            switchForward = false;
        } else {
        	judgeDate(view, date, mNumDays, mSelectionDay, true); // chenzhentao 2012-5-17 modify
            switchForward = true;
        }
        date.normalize(true /* ignore isDst */);
        initView(view);
...
//                    + " mViewStartY: " + mViewStartY);
    ...
    /**
     * chenzhentao 2012-5-17 add
     * @param time
     * @param numDays
     * @param add true means add, otherwise Subtract
     */
    private static void judgeDate(CalendarView view, Time time, int numDays, int selectionDay, boolean forward){
    	System.out.println("CalendarView.java judgeDate(): " + time.year + "-" + time.month + "-" + time.monthDay + ", forward: " + forward);
    	if(forward){
    		if(time.year > MonthView.MAX_YEAR || (time.year == MonthView.MAX_YEAR && time.month == MonthView.MAX_MONTH
    				&& time.monthDay + numDays > MonthView.MAX_MONTH_DAY)){
    			time.year = MonthView.MIN_YEAR;
    			time.month = MonthView.MIN_MONTH;
    			time.monthDay = MonthView.MIN_MONTH_DAY;
    			view.mSelectionDay = MonthView.MIN_MONTH_DAY;
    		} else {
    			time.monthDay += numDays;
    			view.mSelectionDay = selectionDay + numDays;
    		}
    	} else {
    		if(time.year < MonthView.MIN_YEAR || (time.year == MonthView.MIN_YEAR && time.month == MonthView.MIN_MONTH 
    				&& time.monthDay - numDays < MonthView.MIN_MONTH_DAY)){
    			time.year = MonthView.MAX_YEAR;
    			time.month = MonthView.MAX_MONTH;
    			time.monthDay = MonthView.MAX_MONTH_DAY;
    			view.mSelectionDay = MonthView.MAX_MONTH_DAY;
    		} else {
    			time.monthDay -= numDays;
    			view.mSelectionDay = selectionDay - numDays;
    		}
