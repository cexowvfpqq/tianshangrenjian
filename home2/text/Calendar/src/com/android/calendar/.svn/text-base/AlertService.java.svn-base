...
import android.os.PowerManager; //chenzhentao 20120319 add, add screen wake up
import android.os.Process;
import android.os.Vibrator; //chenzhentao 20120314 add, repair calendar's not vibrate bug.
...
public class AlertService extends Service {
    ...
    private static final long VIBRATE_DURATION  = 3000; //chenzhentao 20120314 add, repair calendar's not vibrate bug.
    private static final int SCREEN_WAKEUP_DURATION  = 3500; //chenzhentao 20120319 add, add screen wake up
    ...
    private static void postNotification(Context context, SharedPreferences prefs,
        ...
        // chenzhentao 20120314 add start, repair calendar's not vibrate bug.
		...
		if (prefs.contains(CalendarPreferenceActivity.KEY_ALERTS_VIBRATE_WHEN)) {
			...
			vibrateWhen = prefs.getString(
					CalendarPreferenceActivity.KEY_ALERTS_VIBRATE_WHEN, null);
		} else if (prefs
				.contains(CalendarPreferenceActivity.KEY_ALERTS_VIBRATE)) {
			// No Froyo setting. Migrate pre-Froyo setting to new Froyo-defined
			// value.
			boolean vibrate = prefs.getBoolean(
					CalendarPreferenceActivity.KEY_ALERTS_VIBRATE, false);
		...
		final int ringMode = audioManager.getRingerMode();

		boolean vibrateAlways = vibrateWhen.equals("always");
		boolean vibrateSilent = vibrateWhen.equals("silent");

        boolean isNotesVibrate = false;
        if (vibrateAlways
        		|| (vibrateSilent && ringMode == AudioManager.RINGER_MODE_SILENT)) {
        	isNotesVibrate = true;
        }
        // chenzhentao 20120314 add end, repair calendar's not vibrate bug.
        ...
        if (!quietUpdate) {
            ...
            // chenzhentao 20120314 delete start, repair calendar's not vibrate bug.
            /*
            String vibrateWhen; // "always" or "silent" or "never"
            if(prefs.contains(CalendarPreferenceActivity.KEY_ALERTS_VIBRATE_WHEN))
            {
            ...
            */
            // chenzhentao 20120314 delete end, repair calendar's not vibrate bug.
        ...
        // chenzhentao 20120319 add start, add screen wake up
        PowerManager.WakeLock wakeLockObj = ((PowerManager) context
				.getSystemService(Context.POWER_SERVICE)).newWakeLock(
				PowerManager.FULL_WAKE_LOCK
						| PowerManager.ACQUIRE_CAUSES_WAKEUP
						| PowerManager.ON_AFTER_RELEASE, "");
		if (wakeLockObj != null) {
			wakeLockObj.acquire(SCREEN_WAKEUP_DURATION);
		} else {
			Log.e("chenzhentao",
					"AlertService.java 407 cause an Exception!!!!!!!!!!!");
		}
        // chenzhentao 20120319 add end, add screen wake up
