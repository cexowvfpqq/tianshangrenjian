...
public class DeleteEventHelper {
    ...
    private DialogInterface.OnClickListener mDeleteNormalDialogListener =
        ...
        public void onClick(DialogInterface dialog, int button) {
            ...
            //Modify start on 2012-1-5 for 8174
            updateAlertNotification();
            //Modify end on 2012-1-5 for 8174
            if (mExitWhenDone) {
                mParent.finish();
            }
        }
    ...
    private void updateAlertNotification() {
        ...
        Cursor cursor = CalendarAlerts.query(resolver, new String[] {
                CalendarAlerts._ID
        }, selection, null, null);
        try {
            if (cursor != null && !cursor.moveToFirst()) {
                NotificationManager nm =
                    (NotificationManager) mParent.getBaseContext().getSystemService(
                            Context.NOTIFICATION_SERVICE);
                nm.cancel(0);
            }
            else{
                AlertService.updateAlertNotification(mParent.getBaseContext());
            }
        } finally {
            if (cursor != null) {
                cursor.close();
