...
public class MonthView extends View implements View.OnCreateContextMenuListener {
    ...
    // chenzhentao 2012-5-16 add start, year 1970-2037, month 0-11, monthDay 1-31
    ...
    private void init(MonthActivity activity) {
        ...
        mGestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                ...
                if (velocityY < 0) {
                	judgeMonth(time, true); // chenzhentao 2012-5-16 modify
                } else {
                	judgeMonth(time, false); // chenzhentao 2012-5-16 modify
                }
                time.normalize(true);
    ...
    /**
     * chenzhentao 2012-5-16 add
     * @param time
     * @param forward
     */
    private static void judgeMonth(Time time, boolean forward){
    	if (forward) {
    		if(time.year == MAX_YEAR && time.month == MAX_MONTH){
        		time.year = MIN_YEAR;
        		time.month = MIN_MONTH;
        		time.monthDay = MIN_MONTH_DAY;
        	} else {
        		time.month += 1;
        	}
	    } else {
	    	if (time.year == MIN_YEAR && time.month == MIN_MONTH) {
	    		time.year = MAX_YEAR;
	    		time.month = MAX_MONTH;
	    		time.monthDay = MIN_MONTH_DAY;
	    	} else {
	    		time.month -= 1;
	    	}
